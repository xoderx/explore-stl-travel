...
import { TrendingUp, Users, Wallet, Target, ArrowUpRight, Filter } from 'lucide-react';
...
export function ROIDashboardPage() {
  ...
  const impactData = useMemo(() => {
    return MOCK_ROI_DATA.find(d => d.district === currentDistrict) || MOCK_ROI_DATA[0];
  }, [currentDistrict]);
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Spend Generated', value: `$${impactData.spendGenerated.toLocaleString()}`, icon: Wallet, color: 'text-emerald-500', change: '+12%' },
          { label: 'Foot Traffic', value: impactData.footTraffic.reduce((acc, curr) => acc + curr.count, 0).toLocaleString(), icon: Users, color: 'text-blue-500', change: '+8%' },
          { label: 'Businesses', value: impactData.businessesSupported, icon: Target, color: 'text-amber-500', change: 'Live' },
          { label: 'Active Deals', value: impactData.activeRewards, icon: TrendingUp, color: 'text-purple-500', change: '+5' },
        ].map((stat, i) => (
        ...
        <Card className="border-none shadow-soft bg-card">
          ...
          <CardContent className="h-[300px] flex flex-col justify-center">
            ...
            <div className="mt-4 space-y-2">
              {impactData.categories.map((cat, i) => (

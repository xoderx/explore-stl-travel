  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {stats.map((stat, i) => (
          <div key={i} className="bg-secondary/20 p-6 rounded-3xl border border-border/10 flex flex-col justify-between group hover:bg-secondary/30 transition-colors">
      ...
      <div className="flex justify-end pt-2">
        <Link to="/dashboard">
          <Button variant="ghost" className="rounded-xl font-bold text-orange-500 hover:text-orange-600 gap-2">
            View Full ROI Dashboard <ArrowRight className="w-4 h-4" />
          </Button>
        </Link>
      </div>
    </div>
